  ...
  <view class="pb-5 main">
    ...
    <view class="px-4">
      <view class="scene-box">
        <view
          class="check-teacher mt-2 flex align-item-center justify-content-center"
        >
        ...
        <view class="scene-content">
          <view
            class="content-text mt-3 mr-2"
            @click="$navigateTo(`/pages/virtualCharacter/index`)"
          >
            <text class="table-nowrap-2">{{ personInfo.intro }}</text>
            <view
              class="say-hello mr-2 mt-2 flex align-item-center justify-content-center"
            >
              <image
                :src="`${imageBaseUrl}/6-30-05.png`"
                mode="widthFix"
                class="say-hello-btn"
              ></image>
          ...
          <view class="content-bottom">
            <view class="flex justify-content-between align-item-center px-3">
              <view
                class="description flex flex-direction-column align-item-center justify-content-center"
              >
                <view class="font-weight-bold t-size-36">情景自由聊</view>
                <view class="t-size-22">每天10分钟 进阶口语达人</view>
              </view>
              <button
                v-if="isAuthPhone === true"
              ...
              <button
                ...
                @getphonenumber="
                  getPhone($event, '/pages/virtualCharacter/index')
                "
              >
                <view>开始对练</view>
              </button>
      ...
      >
        ...
        <button
          ...
          type="primary"
          open-type="getPhoneNumber"
          @getphonenumber="getPhone($event, item.toPath)"
        ></button>
      ...
      <view class="flex mt-5 justify-content-between">
        <view
          class="mr-2 mt-3 nots-box flex pt-3 pb-2 flex-direction-column align-item-center"
        >
          ...
          >
            <view class="font-weight-bold t-size-26"
            >重 <span class="t-color-1863E5 t-italic">{{ item.text }}</span>
            </view>
        ...
        <view class="ai-grid-container">
          <view
            class="ai-tools-box mt-3 pl-5 pt-2 pb-2 flex flex-direction-column justify-content-center"
            @click="toProduct(index)"
            v-for="(item, index) in menu2"
            :key="index"
            :style="{ background: item.bgColor }"
          >
            <view class="t-color-3D3D3D font-weight-bold mt-1 t-size-34">{{
                item.text
              }}</view>
            <view class="t-color-3D3D3D t-size-22 mt-2">{{
                item.subText
              }}</view>
            <view class="t-color-3D3D3D t-size-22 mt-05">{{
                item.subText2
              }}</view>

            <image
              :src="`${imageBaseUrl}/${item.image}`"
              :class="index === 1 ? 'image2' : 'image'"
            ></image>
          </view>
        </view>
      </view>
...
export default {
  ...
  onLoad({ promoCode, code }) {
    if (code) this.getSourceCB(code);
    if (promoCode) {
...
.grid-container {
  ...
  .share-box {
    ...
    background: linear-gradient(
        270deg,
        rgba(225, 235, 254, 0.3) 0%,
        rgba(0, 93, 255, 0.3) 100%
    );
